...
/* chkct */
  case 0x332: case 0x33a:
    ...
    { token = intoken(regs[t + op1]);	
	  if (token != (regs[t + op2] | 256))
	  { };	
	};
	break;
...
/* inct */
  case 0x213: case 0x21b:
	op1 = (ops >> 12) & 15; op2 = (ops >> 8) & 15;
	if ((regs[t + op2] & 255) == res_chanend)
	{ token = intoken(regs[t + op2]);	
	  if ((token & 256) == 0)
	  { }
	  else	  
	    regs[t + op1] = token & 255;
	};
	break;
...
/* outct */
  case 0x132: case 0x13a:
	op1 = (ops >> 12) & 15; op2 = (ops >> 8) & 15;	
	if ((regs[t + op1] & 255) == res_chanend)
	  outtoken(regs[t + op1], regs[t + op2], 1);
	break;
...
{ unsigned int token;
  ...
  buffp = chanptr[cnum + ch_buffp] << 3;
  if (buffp > 0)
  { token = (chanbuff[cnum + ch_buff0] & 255) | (chanbuff[cnum + ch_fbuff] & 1);
	chanbuff[cnum + ch_buff0] = (chanbuff[cnum + ch_buff0] >> 8) | (chanbuff[cnum + ch_buff1] & 255);  

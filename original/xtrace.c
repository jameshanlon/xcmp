...
/* then the long immediates */
	...
	{ /* blacp */
	  ...
	  case 0x1cf:		  
		...
		pru6("kcalli", opd);
		prrval(sr);
		prtgt(regs[t + kep] + 64);regs[t + spc] = npc;		 
		break;
	  /* kentsp */
	  case 0x1ee:
		opd = (opd << 6) | (i & 63); 
...
prmem(r, a)
{ proreg(r);
  printf("[%d] = %d", a, mem[(regs[t + r] >> 2) + a]);	
};

prmemr(r, a)
{ proreg(r);
  printf("[r%d] = %d", a, mem[(regs[t + r] >> 2) + regs[t + a]]);	
};

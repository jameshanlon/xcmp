...
func rprocdecls() is 
  var a;
  var b;
{ if (symbol = s_proc) or (symbol = s_func) or (symbol = s_ext)
  then
  { a := rprocdecl();
    while (symbol = s_proc) or (symbol = s_func) or (symbol = s_ext) do
    { b := rprocdecl();
      a := cons3(s_semicolon, a, b)
    }
  }
  ...
  { nextsymbol();
    ...
    { m := rname();
      ...
      { nextsymbol();
        ...
        checkfor(s_begin, "\'{\' expected");
        a := rvardecls();
        b := rprocdecls();
        checkfor(s_end, "\'}\' expected");
        a := cons6(s_moddef, m, a, b, c, 0) 
      }
      else
      { checkfor(s_is, "\'is\' expected");
        checkfor(s_begin, "\'{\' expected");
...
{ if (tree[x + t_op] = s_semicolon)
  ...
  { label := getlabel();
    ...
    if ((tree[x + t_op] = s_extproc) or (tree[x + t_op] = s_extfunc))
    then
    { names_t[namep] := s_extproc;
      names_d[namep] := tree[x + t_op4]
    }
    else   
    { names_t[namep] := n_proc;
      names_d[namep] := makesignature(label, x)
    };
    tree[x + t_op4] := label;
    namep := namep + 1
  }
...
proc declextprocs(val m, val x) is
{ if (tree[x + t_op] = s_semicolon)
  then
  { declextprocs(m, tree[x + t_op1]);
    declextprocs(m, tree[x + t_op2])
  }
  else
  if ((tree[x + t_op] = s_extproc) or (tree[x + t_op] = s_extfunc))
  then
  { names_n[namep] := (m << 16) or tree[x + t_op1];
    names_t[namep] := n_extproc;
    names_d[namep] := makesignature(getextnum(), x);
    tree[x + t_op4] := names_d[namep];
    namep := namep + 1
  }
  else skip
}
...
func getextnum() is  
  var n;
{ n := externalnum;  
  externalnum := externalnum + 1;
  return entry_ext or (currentmodule << 10) or n
}

func external(val e) is
  return (e and entry_ext) ~= 0


func extmod(val e) is
  return (e >> 10) and 1023

func extnum(val e) is
  return (e and 1023)

func extmodref(val m) is
  return 0  


func makesignature(val entry, val x) is
  var i;
  var n;
  var s;
  var f;
  var formals;
{ formals := tree[x + t_op2];
  s := signaturep;
  signatures[signaturep] := entry; 
  signaturep := signaturep + 2;
  n := 0;
  if (formals ~= nil)
  then
  { while (tree[formals + t_op] = s_comma) do
    { f := tree[formals + t_op1];
      signatures[signaturep] := tree[f + t_op];
      signaturep := signaturep + 1;
      formals := tree[formals + t_op2];
      n := n + 1
    };
    signatures[signaturep] := tree[formals + t_op];
    signaturep := signaturep + 1;
    n := n + 1
  }
  else skip;
...
func findextname(val m, val e) is
  return findname((m << 16) or e)
...
{ op := tree[x + t_op];
  ...
  if (op = s_ass) or (op = s_input) or (op = s_output)
  then
  { tree[x + t_op2] := scopenamesexpr(tree[x + t_op2]);
    tree[x + t_op1] := scopenamesexpr(tree[x + t_op1])
  }
...
{ op := tree[x + t_op];
  ...
  { tree[x + t_op4] := livelist;
    decl := tree[x + t_op2];
    dest := tree[x + t_op1];
    sources := fnsourceset(tree[x + t_op3], decl);
    dests := fndestset(tree[x + t_op3], decl); 
    if (dest ~= nil)
    then
    { sources := sunion(sources, lhsourceset(dest));
...
{ if (tree[x + t_op] = s_semicolon)
  ...
  { savetreep := treep;
    ...
    liveformals := fndestset(tree[x + t_op2], sig);
...
func fncheckargs(val x, val s) is
  var n;
  var m;
  var p;
  var f;
...
{ sp := s + 1;
  ...
  { while ((n > 1) and (tree[p + t_op] = s_comma)) do
    { f := signatures[sp];
      a := tree[p + t_op1];
      if (f = s_var) 
      then
      { if ((tree[a + t_op] ~= s_var) and (tree[a + t_op] ~= s_array))
        then
          m := false
        else skip
      }
    ...
    if ((n = 1) and (tree[p + t_op] ~= s_comma))   
    then
    { f := signatures[sp];
      if (f = s_var) 
      then
      { if ((tree[p + t_op] ~= s_var) and (tree[p + t_op] ~= s_array))
        then
          m := false
        else skip
      }
...
func fnsourceset(val x, val s) is
  var sp;
  var n;
  var p;
  var f;
...
{ n := signatures[s + 1];
  ...
  while ((n > 1) and (tree[p + t_op] = s_comma)) do
  { f := signatures[sp];
    if ((f = s_var) or (f = s_valp) or (f = s_chan))
    then
      r := sunion(r, rhsourceset(tree[p + t_op1]))
    else skip;
    p := tree[p + t_op2];
    sp := sp + 1;
    n := n - 1
...
{ n := signatures[s + 1];
  ...
  while ((n > 1) and (tree[p + t_op] = s_comma)) do
  { f := signatures[sp];
    if ((f = s_var) or (f = s_resp))
    then
      r := sunion(r, expdestset(tree[p + t_op1]))
    else skip;
    p := tree[p + t_op2];
    sp := sp + 1;
    n := n - 1
...
proc setresults(val x, val s) is
  var actuals;
  var formal;
  if x = nil
  then skip
  ...
  { actuals := x;
    ...
    { setresult(tree[actuals + t_op1], formal);
      actuals := tree[actuals + t_op2];
      formal := formal + 1; 
      param_count := param_count + 1
    };
    setresult(actuals, formal);
    param_count := param_count + 1
  }

proc setresult(val x, val s) is
  var vn;
{ if ((signatures[s] = s_resp) or (signatures[s] = s_var))
  then
  { vn := vnum(x);
    if (~ ((tree[x + t_op] = s_array) or (tree[x + t_op] = s_chan) or (tree[x + t_op] = s_timer)))
    then
...
{ currentmodule := tree[x + t_op1];
  ...
  declextprocs(currentmodule, tree[x + t_op3]);
...
{ op := tree[x + t_op];
  ...
  { dest := tree[x + t_op1];
    ...
    if tree[dest + t_op] = s_chan
    then 
    { if tree[x + t_op4] = s_master
      then
  ...
  { source := tree[x + t_op1];
    ...
    if tree[source + t_op] = s_chan
    then
    { if tree[x + t_op4] = s_slave
      then
      { geninput(tree[x + t_op2], cchan);
        genrus(i_chkcti, cchan, ct_eom);
        genrus(i_outcti, cchan, ct_eom)
      }
      else 
...
proc gencall(val sig) is 
  var entry;
{ entry := signatures[sig];
  if (external(entry))
  then
  { if (extmod(entry) = currentmodule)
    then
      genru6(i_ldawdp, reg_gdest, 0)
    else
      genru6(i_ldwdp, reg_gdest, extmodref(entry));
    genu6(i_blat, extnum(entry)) 
  } 
  else 
  { if proclabel < signatures[sig]
    then
      genu10ref(i_blrf, signatures[sig], cbf_prog)
    else
      genu10ref(i_blrb, signatures[sig], cbf_prog)
  }
}
...
proc geninput(val list, val rreg) is
  var reg;
  var name;
{ if (tree[list + t_op] = s_comma)
  then
  { geninput(tree[list + t_op1], rreg);
    geninput(tree[list + t_op2], rreg)
  }
  else
    geninassign(list, rreg)
}
...
{ op := tree[x + t_op];
  ...
  if (op = s_var) or (op = s_valp) or (op = s_array)
  then
  { if vn_isset(vnum(x), b_reg) 
    then
      reg := vn_reg(vnum(x))
